...
- Reads commands from control topic                                     *control_str
- Publishes commands to simulated vehicle environment                   *sim_control
- Waits for a specified delay to simulate conrol board processing time
- Waits for a specified delay to simulate control board response time
- Publishes status to topic                                              *control_status
...
Topics: 
 Subscribes to: 
- /vehicle/control_str
    - control commands from other nodes to be sent to the control board, mimics the topic used in the real vehicle for easy integration with conversion layer
 Publishes to: 
- /vehicle/sim_control
    - commands published to the simulated environment to mimic the control board output, mimics the topic used in the real vehicle for easy integration with conversion layer
- /vehicle/control_status
    - status updates from the control board to mimic real control board status output, mimics the topic used in the real vehicle for easy integration with conversion layer
...
- max_command_queue_time (double)
    - max time in seconds that a command can be held in the queue before being dropped to prevent excessive queuing of stale commands
...
class VehicleControlSim(Node):
    def __init__(self):
        ...
        # Publisher for simulated environment
        ...
        # Delay in seconds to simulate control board processing time (callibrate based on hardware)
        # Right now this is just based on the control loop delay in the real vehicle, but does not take into account fluctuations 
        # Simplification for simulation should reflect average processing time of control board
        # Does not include hardware delays such as relay swithing, motor response time, etc.
    ...
    async def process_command_queue(self):
        ...
        while rclpy.ok():
            ...
            # Publish the command to the simulated environment
            sim_msg = String()
            sim_msg.data = f'{command}'
            self.sim_control_publisher.publish(sim_msg)

            # Simulate the response delay
            await asyncio.sleep(self.get_parameter('processing_delay').get_parameter_value().double_value)  # Simulate response delay
