...
class ArduinoSerial:
    ...
    def set_port(self, name,  port, ID=None, baudrate=9600):
        ...
        if name not in self.devices:
            ...
            print(f"Added device {name} on port {port}")
        else:
            self.devices[name].port = port
            print(f"Set port for device {name} to {port}")
    ...
    def open(self, device):
        ...
        if device.port is not None:
            try:
                ...
                print(f"Error opening port {device.port} for device {device.name}: {e}")
                return False
            except Exception as e:
                print(f"Unexpected error opening port {device.port} for device {device.name}: {e}")
                return False
        else: 
            print(f"Port not set for device {device.name}. Cannot open connection.")
            return False
    ...
    def send(self, device_name, data): 
        if device_name in self.devices:
            ...
            try:
                ...
                print(f"Error sending data to {device.name} on port {device.port}: {e}")
                return False
        else:
    ...
    def read(self, device_name, size=64):
        if device_name in self.devices:
            ...
            try:
                ...
                print(f"Error reading data from {device.name} on port {device.port}: {e}")
                return None
        else:
