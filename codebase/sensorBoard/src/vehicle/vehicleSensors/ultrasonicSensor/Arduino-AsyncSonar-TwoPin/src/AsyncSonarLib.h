...
public:
	...
	uint8_t TriggerPin;
	uint8_t EchoPin;					// ADDED FOR TWO-PIN VERSION
	SonarCallback OnPing = nullptr;
	SonarCallback OnTimeOut = nullptr;

#ifndef ASYNCSONAR_USE_SONARISR
	AsyncSonar(uint8_t trigger_pin, uint8_t echo_pin,		// ADDED echo_pin FOR TWO-PIN VERSION
		void(*on_ping)(AsyncSonar&) = nullptr,
		void(*on_time_out)(AsyncSonar&) = nullptr)
		: TriggerPin(trigger_pin), EchoPin(echo_pin), OnPing(on_ping), OnTimeOut(on_time_out) {}
			// ADDED EchoPin FOR TWO-PIN VERSION


// Not modified, do not use for two-pin version without adding echo pin to constructor
#else
