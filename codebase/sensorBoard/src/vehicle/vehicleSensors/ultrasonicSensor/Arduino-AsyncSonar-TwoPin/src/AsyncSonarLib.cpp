...
{
	PcInt::detachInterrupt(EchoPin);		// MODIFIED
	_status = IDLE;
}

void AsyncSonar::responseStart(AsyncSonar *item)
{
	PcInt::detachInterrupt(item->EchoPin);		// MODIFIED
	item->_responseStartMicros = micros();
	PcInt::attachInterrupt(item->EchoPin, responseEnd, item, FALLING);		// MODIFIED
}
...
void AsyncSonar::ping()
{
	...
	digitalWrite(TriggerPin, LOW);
	pinMode(EchoPin, INPUT);                // MODIFIED
	delayMicroseconds(4);
	_status = WAITING_RESPONSE;
	_startTime = millis();
	PcInt::attachInterrupt(EchoPin, responseStart, this, RISING);		// MODIFIED
}
